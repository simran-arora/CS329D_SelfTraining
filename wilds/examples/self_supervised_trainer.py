...
class Trainer():
        ...
        if type(train_data_func) == dict:
            total = sum(
                [
                    ...
                    for _, dataloader in train_data_func.items()
                ]
            )
        else:
            total = len(train_data_func.dataset)
        num_steps_per_epoch = 1 + (total // (self.args.batch_size * accum_steps))
        return self.args.num_epochs * num_steps_per_epoch, total
    ...
    def train(self, model, train_data_func, eval_func):
        ...
        if self.args.get('warmup_steps', 0) > 0:
        ...
        for epoch_num in tqdm(range(num_epochs)):
            ...
            losses = []
            # we don't want to evaluate more than 10 times
            num_losses = len(train_data_func)
            # epoch_size = k * dataset_size and the effective batch size is self.args.batch_size * accum_steps
            EVAL_EVERY = epoch_size // (self.args.batch_size * accum_steps * num_losses)
